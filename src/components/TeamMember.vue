<script setup>
	...
	const laberlColor = computed(() => {
		...
		switch (props.rol) {
			case 'referent':
				className = 'legend-ref'
				break
			case 'coordination':
				className = 'legend-coord'
				break
			case 'islTeam':
				className = 'legend-team'
				break
			default:
				className = 'legend-coord'
		}
...
<template>
	<div class="p-3 mx-auto" style="max-width: 320px">
		<img
			v-if="props.imgUrl"
			:src="`src/assets/images/team-members/${imgUrl}`"
			alt="valentina" />

		<img v-else src="../assets/images/team-members/null-member-2.png" alt="" />

		<div class="text-center py-1 legend" :class="props.rol">
			<h5 class="me-2 text-uppercase">
				{{ title }}

				<a
					v-if="reactiveProps.linkedIn"
					:href="reactiveProps.linkedIn"
					target="_blank">
					<LinkedInIcon />
				</a>
			</h5>
